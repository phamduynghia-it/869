...
const GIFT_INTERVAL = isHighRefresh ? 5000 : 3000;
...
function createSnow() {
  ...
  function fall(t) {
    ...
    snow.style.transform = `translate3d(${x}px,${p *
...
function createGift() {
  ...
  gift.onclick = () => {
    ...
    if (isLetterShown) {
      ...
      currentImageIndex =
        (currentImageIndex + 1) % randomImages.length;
...
popupImg.onclick = () => {
  ...
  setTimeout(() => {
    ...
    setTimeout(() => {
      showGiftMessage(
        `Merry Christmas🎄
Nay là mùa giáng sinh đầu tiên chúng ta ở bên nhau. Tui mong những điều tốt đẹp nhất sẽ đến với bạn. Cầu nguyện cho người con trai mà tui thích lúc nào cũng hạnh phúc, luôn nở nụ cười thật tươi nhaa. Chúc bạn giáng sinh an lành và thật ấm áp. 
Và điều cuối cùng, hi vọng giáng sinh năm sau chúng ta vẫn có thể ở cạnh nhau, bạn nhé ❤️`
      );
    }, 500);
  }, 300);
};
